...
- [ ] Add note to SDE description noting when layer will be deleted and any replacement layer (name, completed: `2026/00/00`)
...
- [ ] Change `Authoritative` field to `d` in `SGID.META.AGOLItems` to automatically set the `Deprecated` AGOL flag. Allow the `d` to persist through one run of [Auditor](https://github.com/agrc/auditor) - currently, Auditor runs daily at 5:00am (name, completed: `2026/00/00`)
  - After at least one successful run of Auditor:
    - [ ] Remove the row from the `SGID.META.AGOLItems` table. This will trigger the removal of this item in Open SGID (name, completed: `2026/00/00`)
    - [ ] Unshare the item from its SGID group, which will remove it from the SGID on ArcGIS (name, completed: `2026/00/00`)
...
- [ ] Manually remove data from the Internal SGID (name, completed: `2026/00/00`)
  - [ ] Deprecated database layers should be backed up on Google Drive > AGRC Projects > SGID > deprecated layers > [internal_db_sgid_layers](https://drive.google.com/drive/u/0/folders/10Fk8NI2UpEUnAbgvhjlN18pyvSFDWyIq) (name, completed: `2026/00/00`).
- [ ] Remove ArcGIS Online item: (name, completed: `2026/00/00`)
...
- [ ] Remove Farm from AGOL connection (name, completed: `2026/00/00`)
...
- [ ] Update `SGID.META.AGOLItems` table (name, completed: `2026/00/00`)
...
- [ ] Remove row from `SGID.META.ChangeDetection` (name, completed: `2026/00/00`)
...
- [ ] Remove data from record series with archives (@eneemann, completed: `2026/00/00`)
- [ ] Remove entry from [MATT](https://github.com/agrc/metadata-asset-tracking-tool/tree/main/metadata) (name, completed: `2026/00/00`)

#### Shelve/Static

_Choose one based on situation._

- [ ] Upload to `UtahAGRC/AGRC_Shelved` folder in AGOL (New shelved item not already in AGOL) (name, completed: `2026/00/00`)
- [ ] Move existing AGOL item to `AGRC_Shelved` AGOL folder (shelving an item already in AGOL) (name, completed: `2026/00/00`)
- [ ] Upload to appropriate `UtahAGRC/{SGID Category}` folder in AGOL (for `static` datasets) (name, completed: `2026/00/00`)

_Add record to table._

- [ ] Add record to `AGOLItems_shelved` [table](https://utah.maps.arcgis.com/home/item.html?id=1760fbedbc7e49429aa6c0c3ab1442ec) in ArcGIS Online with `shelved` or `static` in the `CATEGORY` field (name, completed: `2026/00/00`)
...
- [ ] Notify Don Jackson (don.jackson1 at motorolasolutions.com) if the data is in the Next-Generation 911 Motorola Aware Map? (@mhchou on `2026/00/00`)
