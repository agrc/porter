...
def test_metatable_checker_grade_integration_add_success(mocker):
    ...
    Grade = namedtuple("Grade", "check grade issue")
    grade = MetaTableChecker("table.name", {}).grade(
        add=True,
        report_value=MetaResponse(True, "item id", "item name", "geometry type"),
    )
...
def test_write_reports(mocker):
    ...
    mocker.patch(
        "conductor.checks.MetaTableChecker.exists",
        return_value=MetaResponse(True, "item_id", "item_name"),
    )
    ...
    reports = conductor.write_reports(
        [
            ...
            ConductorIssue(GitHubIssue("issue body text", 1, "issue title"), True),
            ConductorIssue(
                GitHubIssue("issue body text with - [ ] a task (@stdavis)", 1, "issue title"),
                True,
            ),
        ],
        SECRETS,
...
def test_write_report_without_item_id(mocker):
    ...
    conductor.extract_metadata_from_issue_body = mocker.MagicMock(
        side_effect=[None, {"no table": ""}, {"table": "fake.table"}]
    )
    ...
    mocker.patch(
        "conductor.checks.MetaTableChecker.exists",
        return_value=MetaResponse(False, None, None),
    )
    mock = mocker.patch("conductor.checks.MetaTableChecker.data", new_callable=mocker.PropertyMock)
...
def test_startup_with_no_issues_returns(mocker):
    ...
    conductor.startup({"github_token": "token"}, False)
